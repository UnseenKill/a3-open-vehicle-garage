...
private _config = configFile >> QADDON;

try {
    if !assert(isClass _config) then {
        throw format["Something is seriously wrong. No %1 config found.", QADDON];
    };

    if (getNumber(_config >> "scope") == 0) then {
        throw format["Nothing overrides %1 config; have to assume nothing is set up.",QADDON];
    };
